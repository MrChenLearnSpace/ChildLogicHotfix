...
namespace ChildLogicHotfix {
    ...
    public partial class HandleConnMsg {
        ...
        public void MsgLogin(Conn conn, ProtocolBase protoBase) {
            ...
            var mongo = ((Mongo)DataMgr.instance.database).database.GetCollection<AccountData>("accounts");
            FilterDefinition<AccountData> filterDefinition = Builders<AccountData>.Filter.Eq("id", id);
            AccountData accountData = mongo.Find(filterDefinition).ToList()[0];
            if(accountData.isBanned) {
                if(accountData.banEndTime<Sys.GetTimeStamp()) {
                    UpdateDefinition<AccountData> updateDefinition = Builders<AccountData>.Update.Set("isBanned", false);
                    mongo.UpdateOne(filterDefinition, updateDefinition);
                }else {
                    msgLogin.result = -1;
                    protocol = msgLogin.Encode();
                    conn.Send(protocol);
                    Console.WriteLine(strFormat + " 用户名：" + id + " 被封号");
                    return;
                }
            }
